...
def show_login():
    ...
    ir_a_registro = params.get("registro", "")
    warning_msg   = st.session_state.pop("login_warning", "")
    ...
    <style>
      ...
      [data-testid="stTextInput"] input { font-family: 'DM Sans', sans-serif !important; font-size: 14px !important; border: 0.5px solid #d0d0d0 !important; border-radius: 8px !important; padding: 12px 14px !important; color: #1a1a2e !important; box-shadow: none !important; }
      [data-testid="stTextInput"] input:focus { border-color: #D85A30 !important; box-shadow: none !important; }
      [data-testid="stTextInput"] label { font-size: 11px !important; font-weight: 500 !important; letter-spacing: 0.08em !important; text-transform: uppercase !important; color: #888 !important; }
      div[data-testid="stButton"]:nth-of-type(1) button { width: 100% !important; padding: 13px !important; background: #1a1a2e !important; color: #f0ede8 !important; border: none !important; border-radius: 8px !important; font-size: 14px !important; font-weight: 500 !important; }
      div[data-testid="stButton"]:nth-of-type(1) button:hover { background: #2a2a4e !important; }
      div[data-testid="stButton"]:nth-of-type(2) button { width: 100% !important; padding: 12px !important; background: transparent !important; color: #888 !important; border: 0.5px solid #d0d0d0 !important; border-radius: 8px !important; font-size: 13px !important; }
...
""")
          ...
          <style>
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .left-tagline h2 {
              ...
              color: #f0ede8; line-height: 1.45; margin: 0 0 14px;
            }
            .left-tagline p {
              font-size: 14px; color: #8b8b9e;
    ...
    # ── Panel DERECHO (Streamlit nativo) ───────────────────────────────────
    with col_right:
        st.markdown('<div class="right-panel">', unsafe_allow_html=True)
        st.markdown('<p class="login-title">Bienvenido de nuevo</p>', unsafe_allow_html=True)
        st.markdown('<p class="login-subtitle">Ingresa tu correo para acceder a tu cuenta</p>', unsafe_allow_html=True)

        if warning_msg:
            st.markdown(f'<div class="form-warning">{warning_msg}</div>', unsafe_allow_html=True)

        correo = st.text_input("Correo electrónico", placeholder="nombre@empresa.com", key="login_correo")

        if st.button("Continuar →", key="btn_login"):
            if not correo:
                st.session_state.login_warning = "Por favor ingresa tu correo electrónico."
                st.rerun()
            else:
                session, mensaje = handle_login(correo)
                if session:
                    st.session_state.session = session
                    st.session_state.correo   = correo
                    st.session_state.page = "dashboard"
                    st.rerun()
                elif "No account found" in mensaje:
                    st.session_state.login_warning = "No account found."
                    st.rerun()
                else:
                    st.session_state.login_warning = mensaje
                    st.rerun()

        st.markdown("""
        <div class="divider-wrap">
          <div class="divider-line"></div>
          <span class="divider-text">¿Primera vez aquí?</span>
          <div class="divider-line"></div>
        </div>
        """, unsafe_allow_html=True)

        if st.button("Crear una cuenta nueva", key="btn_register"):
            st.session_state.page = "signup"
            st.rerun()
